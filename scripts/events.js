...
serchInput.addEventListener("change", () => console.log(serchInput.value))

const findTab = () => {
    animalTabs.forEach(tab => {
        if (tab.textContent.toLowerCase().contains(serchInput.value)) {
...
serchInput.addEventListener("input", findTab)
...
let Australia = {
    ...
    langauge: "English",
    population: 25000000,
    sport: ["Ceicket", "Australia Rules", "Tennis"]
}

console.log(`there are  ${australia.population} pepole livin in ${australia.name}`)

function Country (name, capitalCity, langauge, population, sport) {
    this.name = name;
    this.capitalCity = capitalCity;
    this.langauge = langauge;
    this.population = population;
    this,sport = sport;
    this.discription = () => console.log(`${this.name}`)
}

let sweden = new Country("sweden","Stockholm", "svenska", 11000000, ["hockey", "Brandboll", "Bandy"] );
let pakistan = new Country("Pakistan", "Islamabad", "urdu", 241000000, ["Criket", "Hocky"]);

console.log(sweden.capitalCity)
console.log(pakistan.langauge)
